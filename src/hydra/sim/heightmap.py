...
def _generate_heightmap_equirect(
    obj: bpy.types.Object,
    size: tuple[int, int] | None = None,
    normalized: bool = False,
    world_scale: bool = False,
    ...
    vao_polar, vao_equirect = model.create_vaos(
        ctx, [common.data.programs["polar"], common.data.programs["equirect"]], obj
    )
    ...
    groups = (
        math.ceil(size[0] / 32),
        math.ceil(0.25 * size[1] / 32),
    )  # only half the hemisphere is mapped from polar, one hemisphere at a time
...
def generate_heightmap(
    ...
    if equirect:
        txt = _generate_heightmap_equirect(
            obj, size, normalized, world_scale, local_scale, internal
        )
    else:
...
def generate_heightmap_from_image(img: bpy.types.Image) -> mgl.Texture:
    ...
    :rtype: :class:`moderngl.Texture`"""
    pixels = (
        np.array(img.pixels).astype("f4")[::4].copy()
    )  # has to be contiguous in memory
...
def prepare_heightmap(obj: bpy.types.Image | bpy.types.Object) -> None:
    ...
    reload = not data.has_map(
        hyd.map_base
    )  # no base or invalid data -> reload completely
...
def subtract(
    modified: mgl.Texture, base: mgl.Texture, factor: float = 1.0, scale: float = 1.0
) -> mgl.Texture:
...
def get_displacement(obj: bpy.types.Object, name: str) -> bpy.types.Image:
    ...
    target = add(
        data.get_map(hyd.map_result).texture,
...
def set_result_as_source(
    obj: bpy.types.Object | bpy.types.Image, as_base: bool = False
) -> None:
...
def add_subres(
    height: mgl.Texture, height_prior: mgl.Texture, height_prior_fullres: mgl.Texture
) -> mgl.Texture:
...
def rotate_equirect_to(
    equirect: mgl.Texture,
    target: mgl.Texture,
    bind_target_to: int,
    sampler_use: int = 1,
    backwards: bool = False,
) -> mgl.Texture:
    shader = common.data.shaders[
        "rotate_equirect" if equirect.components == 1 else "rotate_equirect_rgba"
    ]
    ctx = common.data.context
...
def recover_heightmaps(obj: bpy.types.Object):
    if (
        apply.PREVIEW_MOD_NAME in obj.modifiers
        and apply.PREVIEW_DISP_NAME in bpy.data.images
    ):
        ...
        diff = texture.create_texture(
            base.size, image=bpy.data.images[apply.PREVIEW_DISP_NAME], channels=1
        )
        ...
        obj.hydra_erosion.map_base = common.data.create_map(
            "Base map", base, logarithmic=planet
        )
        obj.hydra_erosion.map_source = common.data.create_map(
            "Recovered", source, logarithmic=planet
        )
        ...
        if diff_name in bpy.data.images:
            ...
            diff = texture.create_texture(
                base.size, image=bpy.data.images[diff_name], channels=1
            )
            ...
            if "HYD_Displacement" in m.node_group.nodes:
                ...
                if (
                    node.type == "IMAGE_TEXTURE"
                    and node.inputs["Image"].default_value is not None
                ):
                    common.data.add_message(
                        f"Found texture '{node.inputs['Image'].default_value.name}' to recover from."
                    )
                    diff = texture.create_texture(
                        base.size, image=node.inputs["Image"].default_value, channels=1
                    )
        ...
        if diff is not None:
            ...
            common.data.add_message(
                f"Couldn't find Hydra's heightmap node (HYD_Displacement) in group.",
                error=True,
            )
            base.release()
        ...
        common.data.add_message(
            "Couldn't find any Hydra modifier on this object.", error=True
        )
