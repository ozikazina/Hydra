...
def erode(obj: bpy.types.Image | bpy.types.Object) -> None:
    ...
    progA["alpha"] = (
        math.tan(hyd.thermal_angle) * 2 / size[0]
    )  # images are scaled to 2 z/x -> angle depends only on image width
    if planet:
    ...
    progA["maxDif"] = hyd.thermal_max_drop
    ...
    group_x = math.ceil(size[0] / WG_SIZE_X)
    group_y = math.ceil(size[1] / WG_SIZE_Y)
