...
def erode(obj: bpy.types.Object | bpy.types.Image) -> None:
    ...
    temp = texture.create_texture(
        size
    )  # capacity, water and sediment at different stages

    if hyd.erosion_hardness_src in bpy.data.images:
        hardness = texture.create_texture(
            size, channels=1, image=bpy.data.images[hyd.erosion_hardness_src]
        )
    else:
        hardness = None

    if hyd.mei_water_src in bpy.data.images:
        water_src = texture.create_texture(
            size, channels=1, image=bpy.data.images[hyd.mei_water_src]
        )
    else:
        water_src = None
    ...
    sedimentSampler = ctx.sampler(
        texture=temp, repeat_x=tile_x, repeat_y=tile_y
    )  # sediment will be in temp at stage 6
    temp.use(LOC_SEDIMENT)
    sedimentSampler.use(LOC_SEDIMENT)

    WORKGROUP_SIZE = 16

    group_x = math.ceil(size[0] / WORKGROUP_SIZE)
    group_y = math.ceil(size[1] / WORKGROUP_SIZE)
    ...
    progs[4]["Ks"] = (
        1 - (1 - (hyd.mei_hardness / 100 - 1) ** 2) ** 0.15
    )  # maps interval 0.5-1.0 to hardness 0.9-1.0
    ...
    for i in range(hyd.mei_iter_num * 10):
        ...
        ctx.memory_barrier(ctx.SHADER_IMAGE_ACCESS_BARRIER_BIT)
