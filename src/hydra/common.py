...
class HydraData(object):
    ...
    def init_context(self):
        ...
        force_get_context = get_preferences().force_get_context
        is_vulkan_backend = (
            hasattr(bpy.context.preferences, "system")
            and hasattr(bpy.context.preferences.system, "gpu_backend")
            and bpy.context.preferences.system.gpu_backend == "VULKAN"
        )

        if is_vulkan_backend and not force_get_context:
            self.context = mgl.create_context(standalone=True)
        else:
            self.context = (
                mgl.get_context()
            )  # standalone crashes blender; create_context doesn't work with wayland
