...
class LandscapeOperator(ops_common.ImageOperator):
    ...
    def execute(self, ctx):
        ...
        if hyd.tiling == "planet":
            apply.add_planet(
                target,
                max_verts_per_side=hyd.planet_resolution,
                name=name,
                detach=self.detach,
            )
        else:
            apply.add_landscape(
                target,
                max_verts_per_side=hyd.landscape_resolution,
                name=name,
                detach=self.detach,
                tile=hyd.tiling != "none",
            )
...
class OverrideImageOperator(ops_common.ImageOperator):
    ...
    def invoke(self, ctx, event):
        ...
        texture.write_image(
            target.name, common.data.get_map(target.hydra_erosion.map_result).texture
        )
