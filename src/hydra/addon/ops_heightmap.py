...
class MergeOp(ops_common.HydraOperator):
    ...
    def poll(cls, ctx):
        ret = (
            not ctx.object.data.shape_keys
            or len(ctx.object.data.shape_keys.key_blocks) == 0
        )
        ...
        return ret and any(
            ...
            if m.show_viewport
            and (m.name.startswith("HYD_") or m.name.startswith("HYDP_"))
        )
    ...
    def execute(self, ctx):
        hydra_mods = [
            m
            for m in ctx.object.modifiers
            if m.name.startswith("HYD_") or m.name.startswith("HYDP_")
        ]
...
class MoveOp(ops_common.HydraOperator):
    ...
    bl_idname = "hydra.hm_move"
    bl_label = "Set as Source"
...
class MoveBackOp(ops_common.HydraOperator):
    ...
    bl_idname = "hydra.hm_move_back"
    bl_label = "Set as Result"
...
class ModifierOp(ops_common.HydraOperator):
    ...
    def invoke(self, ctx, event):
        ...
        apply.remove_preview()
        displacement = heightmap.get_displacement(
            target, name=f"HYD_{target.name}_DISPLACE"
        )
