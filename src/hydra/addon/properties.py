...
class ErosionGroup(bpy.types.PropertyGroup):
    ...
    height_scale: FloatProperty(name="Height scale", default=1)
    """Height scaling factor after normalization (Z/X). Value of 1 means same scale as heightmap width."""

    scale_ratio: FloatProperty(name="Scale ratio", default=1)
    """Ratio of Y to X scales for non-uniform images."""

    org_scale: FloatProperty(name="Original height scaling", default=1)
    """Original height scaling to use with modifiers, which affect it. (dim.z/scale.z)"""
    ...
    def is_planet(self):
        return self.tiling == "planet"

    advanced: BoolProperty(
        default=False, name="Advanced", description="Show advanced settings"
    )
    ...
    erosion_solver: EnumProperty(
        ...
        items=(
            (
                "particle",
                "Particle",
                "Creates long meandering paths. Stable for most terrains",
                0,
            ),
    ...
    thermal_max_drop: FloatProperty(
        default=1e-1,
        min=1e-5,
        max=1e-1,
        name="Maximum drop",
        description="Maximum terrain drop per iteration. Produces a wet sand/goo effect at very low values.",
    )
    ...
    color_src: StringProperty(name="Color", description="Color image texture")
    ...
    is_generated: BoolProperty(
        name="Is generated",
        description="Prevents resource allocation for generated objects",
    )
    ...
    heightmap_gen_type: EnumProperty(
        ...
        items=(
            (
                "normalized",
                "Normalized",
                "Scales heightmap to the range [0,1], 1 (white) being highest",
                0,
            ),
            ...
            (
                "local",
                "Local size",
                "Preserves object height (without object scale applied)",
                2,
            ),
