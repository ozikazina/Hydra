...
class HydraPanel(bpy.types.Panel):
    ...
    def draw_nav_fragment(self, container, name, label):
        if name in bpy.data.images:
            ...
            split.operator("hydra.nav_img", text="", icon="TRIA_RIGHT_BAR").target = (
                name
            )
        ...
        if common.data.has_map(settings.map_base):
            ...
            if common.get_preferences().direct_resolution:
                ...
                if settings.tiling == "planet":
                    c.label(
                        text=f"({2 * settings.img_size[1]}, {settings.img_size[1]}) px"
                    )
                else:
                    c.label(text=f"({settings.img_size[0]}, {settings.img_size[1]}) px")
...
class ErosionPanel:
    ...
    def draw(self, ctx):
        ...
        if common.data.has_map(hyd.map_result):
            grid.operator("hydra.erode", text="Set & Continue", icon="ANIM").apply = (
                True
            )
...
class ThermalPanel:
    ...
    def draw(self, ctx):
        ...
        if common.data.has_map(hyd.map_result):
            grid.operator("hydra.thermal", text="Set & Continue", icon="ANIM").apply = (
                True
            )
    ...
    def draw(self, ctx):
        ...
        if common.data.has_map(hyd.map_result):
            ...
            if isinstance(target, bpy.types.Image):
                ...
                if (
                    apply.PREVIEW_IMG_NAME in bpy.data.objects
                    or apply.PREVIEW_IMG_NAME in bpy.data.images
                ):
                ...
                cols.operator(
                    "hydra.override_original", text="", icon="IMAGE_REFERENCE"
                )
            else:
                box = col.box()
                split = box.split(factor=0.5)
                ...
                m = next(
                    (m for m in target.modifiers if m.name.startswith("HYD_")), None
                )
                if m:
                    if m.type == "DISPLACE":
                        cols = box.column_flow(columns=2, align=True)
                        ...
                        cols.operator(
                            "hydra.hm_merge_shape", text="", icon="SHAPEKEY_DATA"
                        )
                    else:
        ...
        if not has_any:
            ...
            if (
                ...
                and hyd.map_base != ""
                and not common.data.has_map(hyd.map_base)
            ):
                # invalid cache
    ...
    def draw(self, ctx):
        ...
        if owner := nav.get_owner(target.name):
            ...
            if owner in bpy.data.objects:
                split.operator(
                    "hydra.nav_obj", text="", icon="TRIA_RIGHT_BAR"
                ).target = owner
            elif owner in bpy.data.images:
                split.operator(
                    "hydra.nav_img", text="", icon="TRIA_RIGHT_BAR"
                ).target = owner
            else:
