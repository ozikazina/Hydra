...
def get_or_make_image(
    size: "tuple[int,int]", name: str
    ...
    if name not in bpy.data.images:
        img = bpy.data.images.new(
            name, size[0], size[1], alpha=False, float_buffer=True, is_data=True
        )
    else:
...
def write_image(
    name: str, texture: mgl.Texture, exp_convert: bool = False
) -> "tuple[bpy.types.Image, bool]":
...
def clone(txt: mgl.Texture) -> mgl.Texture:
    ...
    :return: Created texture.
    :rtype: :class:`moderngl.Texture`"""
    return common.data.context.texture(
        txt.size, txt.components, dtype="f4", data=txt.read()
    )
...
def get_min_max(txt: mgl.Texture) -> "tuple[float,float]":
    ...
    prog["height"] = size[1]
    THREAD_COUNT = 256
    prog.run(math.ceil(size[0] / THREAD_COUNT), 1)
