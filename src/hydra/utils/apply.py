...
def add_planet(img, max_verts_per_side=512, name: str|None = None, detach: bool = False):
	...
	if not detach and name in bpy.data.objects:
		...
		verts_per_side = res ** 2
		faces_per_side = (res-1) ** 2
		...
		start = np.array(range(0, res * (res - 1), res), dtype=np.int32).repeat(res-1).reshape((res-1,res-1)) + np.array([range(res-1)], dtype=np.int32)
		# Repeats each vertex 4 times, adds 0,1 (bottom edge), res+1,res (top edge) to make a face
		faces[:end] = start.reshape(-1).repeat(4).reshape(((res-1)**2, 4)) + np.array([[0,1,res+1,res]], dtype=np.int32)
		offset = end
		vert_offset = verts_per_side

		# Bottom face
		end += faces_per_side
		start = np.array(range(0, res * (res - 1), res), dtype=np.int32).repeat(res-1).reshape((res-1,res-1)) + np.array([range(res-1)], dtype=np.int32)
		faces[offset:end] = start.reshape(-1).repeat(4).reshape(((res-1)**2, 4)) + (np.array([[res,res+1,1,0]], dtype=np.int32) + verts_per_side)
		offset = end
		vert_offset += verts_per_side

		# -Y
		end += (res-3)*(res-1)
		start = np.array(range(vert_offset, res * (res - 3) + vert_offset, res)).repeat(res-1).reshape((res-3,res-1)) + np.array([range(res-1)])
		faces[offset:end] = start.reshape(-1).repeat(4).reshape(((res-1)*(res-3), 4)) + np.array([[0,1,res+1,res]])
		offset = end

		end += res - 1
		stride = vert_offset + res * (res - 3)
		faces[offset:end] = [[x + stride, x + 1 + stride, x + 1, x] for x in range(0, res-1)]
		offset = end

		end += res - 1
		stride = vert_offset
		stride2 = res ** 2
		faces[offset:end] = [[x + stride2, x + 1 + stride2, x + 1 + stride, x + stride] for x in range(0, res-1)]
		offset = end
		...
		start = np.array(range(vert_offset, res * (res - 3) + vert_offset, res)).repeat(res-1).reshape((res-3,res-1)) + np.array([range(res-1)])
		faces[offset:end] = start.reshape(-1).repeat(4).reshape(((res-1)*(res-3), 4)) + np.array([[res,res+1,1,0]])
		offset = end
		...
		stride = vert_offset
		stride2 = res ** 2 + res * (res - 1)
		faces[offset:end] = [[x + stride, x + 1 + stride, x + 1 + stride2, x + stride2] for x in range(0, res-1)]
		offset = end
		vert_offset += res * (res - 2)

		# -X
		end += (res-3)**2
		start = np.array(range(vert_offset, (res-2) * (res - 3) + vert_offset, res-2)).repeat(res-3).reshape((res-3,res-3)) + np.array([range(res-3)])
		faces[offset:end] = start.reshape(-1).repeat(4).reshape(((res-3)**2, 4)) + np.array([[res-2,res-1,1,0]])
		offset = end

		end += res - 3
		stride = vert_offset
		stride2 = vert_offset - 2 * res * (res - 2)
		faces[offset:end] = [[x * res + stride2, (x + 1) * res + stride2, (x + 1) * (res - 2) + stride, x * (res - 2) + stride] for x in range(0, res-3)]
		offset = end

		end += res - 3
		stride = vert_offset + res - 3
		stride2 = vert_offset - res * (res - 2)
		width1 = res - 2
		width2 = res
		faces[offset:end] = [[x * width1 + stride, (x + 1) * width1 + stride, (x + 1) * width2 + stride2, x * width2 + stride2] for x in range(0, res-3)]
		offset = end

		end += res - 3
		stride = res
		stride2 = vert_offset + (res - 2) * (res - 3)
		width1 = res
		width2 = 1
		faces[offset:end] = [[x * width1 + stride, (x + 1) * width1 + stride, (x + 1) * width2 + stride2, x * width2 + stride2] for x in range(0, res-3)]
		offset = end
		...
		faces[offset:end] = [
			...
			[
				res * (res - 2),
				res * (res - 1),
				vert_offset - res * (res - 2) + res * (res - 3),
		...
		start = np.array(range(vert_offset, (res-2) * (res - 3) + vert_offset, res-2)).repeat(res-3).reshape((res-3,res-3)) + np.array([range(res-3)])
		faces[offset:end] = start.reshape(-1).repeat(4).reshape(((res-3)**2, 4)) + np.array([[0,1,res-1,res-2]])
		offset = end

		end += res - 3
		stride = vert_offset
		stride2 = vert_offset - 3 * res * (res - 3) - 5
		faces[offset:end] = [[
			x * (res - 2) + stride,
			(x + 1) * (res - 2) + stride,
			(x + 1) * res + stride2,
		...
		stride = vert_offset + res - 3
		stride2 = vert_offset - 2 * res * (res - 3) - 5 + res
		width1 = res - 2
		width2 = res
		faces[offset:end] = [[
			x * width2 + stride2,
		...
		faces[offset:end] = [
			...
			[
				vert_offset + (res - 2) * (res - 3) + res - 3,
				vert_offset - res * (res - 2) + res * (res - 3) - res * (res - 5) - 5,
				res * (res - 1) + res-1,
				res * (res - 2) + res-1
			]
		]
		...
		act = bpy.data.objects.new("Test", mesh)
